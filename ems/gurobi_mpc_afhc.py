...
class GurobiMPC(Manager):
    def __init__(self, fixed_steps, weight_step="equal", steps_skip=1, file_name=None):
        ...
        self.steps_cache = [deque() for _ in range(7)]
    ...
    def calculate_powers(self, observation, forec_scenarios, time_step):
        ...
        for b in range(num_batts):
            if len(self.steps_cache[b]) >= self.steps_skip:
                # pop the oldest element in each list of the cache
                self.steps_cache[b].popleft()
            else:
                # do nothing
                pass
        ...
        for b in range(num_batts):
            ...
            # Cache actions
            action = [pb / batt_capacity[b] for pb in power_batteries[b]]
            self.steps_cache[b].append(action)

            action_sum = 0
            count = 0

            iteration = min(time_step, (self.steps_skip - 1)) 
            for i in range(min(iteration + 1, len(self.steps_cache[b]))):
                action_sum += self.steps_cache[b][i][iteration - i]
                count += 1

            action = action_sum / count if count > 0 else 0
